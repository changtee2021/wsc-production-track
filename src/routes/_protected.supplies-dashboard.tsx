...
import {
  adminListOfficeRequests,
  adminApproveOfficeRequest,
  adminRejectOfficeRequest,
  adminOfficeRestock,
  adminOfficeStockDashboard,
  officeListEmployees,
...
function DashboardPage() {
  ...
  const [summary, setSummary] = useState<{
    ...
    total_spend: number;
    low_stock: Low[];
    top_consumed: { id: string; name: string; code: string; qty: number }[];
  } | null>(null);
    ...
    <main className="mx-auto max-w-5xl space-y-4 p-4">
          ...
          {summary.low_stock_count > 0 && (
            <div className="flex items-center gap-2 rounded-md border border-rose-300 bg-rose-50 px-3 py-2 text-sm text-rose-900 dark:border-rose-900/50 dark:bg-rose-950/40 dark:text-rose-200">
              ...
              <span>
                {outOfStock > 0 && (
                  <>
                    <b>{outOfStock}</b> รายการ <b>หมดสต๊อก</b> ·{" "}
                  </>
                )}
                {nearLow > 0 && (
                  <>
                    <b>{nearLow}</b> รายการ <b>ใกล้หมด</b> ·{" "}
                  </>
                )}
                ต้องสั่งซื้อเพิ่ม
              </span>
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        <StatCard
          ...
          tone={summary.pending_count > 0 ? "primary" : undefined}
        />
        <StatCard
          label="ของใกล้หมด"
          value={summary.low_stock_count}
          icon={<AlertTriangle className="h-4 w-4" />}
          tone={summary.low_stock_count > 0 ? "warn" : undefined}
        />
        <StatCard
          label={`ค่าใช้จ่ายเดือนนี้`}
          value={`฿${summary.month_spend.toLocaleString()}`}
      ...
      <section className="space-y-2">
        ...
        {requests.length === 0 ? (
          ...
          requests.map((r) => <RequestCard key={r.id} req={r} emps={emps} onDone={refresh} />)
        )}
      </section>
                  ...
                  <CardContent className="flex items-center gap-3 p-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        ...
                        <b className={out ? "text-rose-600" : "text-amber-700 dark:text-amber-400"}>
                          {a.stock_qty}
                        </b>{" "}
...
function StatCard({
  ...
  const color =
    tone === "primary" ? "text-primary" : tone === "warn" ? "text-amber-600" : "text-foreground";
  return (
    <Card>
...
function RequestCard({ req, emps, onDone }: { req: Req; emps: Emp[]; onDone: () => void }) {
  ...
  const doApprove = async () => {
    if (!approverId) {
      toast.error("เลือกผู้อนุมัติก่อน");
      return;
    }
    setBusy(true);
    try {
      await approve({
        data: { token: requireToken(), id: req.id, approver_employee_id: approverId },
      });
    ...
    try {
      await reject({
        data: {
          token: requireToken(),
          id: req.id,
          approver_employee_id: approverId || undefined,
          ...
          {req.items.map((it) => (
            <div key={it.id} className="flex items-center gap-2">
              ...
              <span className="w-20 text-right text-muted-foreground">
                ฿{(Number(it.unit_price_snapshot) * it.qty).toLocaleString()}
              </span>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="min-w-[180px] flex-1">
            <Select value={approverId} onValueChange={setApproverId}>
              <SelectTrigger>
                <SelectValue placeholder="ผู้อนุมัติ (พนักงานออฟฟิศ)" />
              </SelectTrigger>
              <SelectContent>
                {emps
                  .filter((e) => e.active)
...
function RestockDialog({ asset, onClose }: { asset: Low; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    try {
      await restock({
        data: { token: requireToken(), asset_id: asset.id, qty, note: note.trim() || undefined },
      });
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <DialogTitle>เติมสต๊อก: {asset.name}</DialogTitle>
        </DialogHeader>
        <div className="space-y-2">
          <div className="text-sm text-muted-foreground">
            คงเหลือปัจจุบัน: {asset.stock_qty} {asset.unit}
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground">
              จำนวนที่เติม ({asset.unit})
            </label>
            <Input
              type="number"
              min={1}
        ...
        <DialogFooter>
          ...
          <Button disabled={busy} onClick={save}>
            {busy && <Loader2 className="mr-1 h-4 w-4 animate-spin" />}บันทึก
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
