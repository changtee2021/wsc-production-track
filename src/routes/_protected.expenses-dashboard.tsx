...
import { Loader2, CheckCircle2, XCircle, Wallet, AlertTriangle, ImageIcon } from "lucide-react";
...
import {
  ...
  adminExpenseApprove,
  adminExpenseReject,
  adminExpenseMarkPaid,
  adminExpenseBulkApprove,
...
const BILL_LBL: Record<string, string> = {
  cash: "เงินสด",
  short_tax: "ใบกำกับย่อ",
  full_tax: "ใบกำกับเต็ม",
};
...
function Page() {
  ...
  const [status, setStatus] = useState<"pending" | "approved" | "rejected" | "paid" | "all">(
    "pending",
  );
...
function ExpenseCard({
  ...
  const doApprove = async () => {
    ...
    try {
      await approve({
        data: { token: requireToken(), id: row.id, approver_employee_id: approverId },
      });
      toast.success(`อนุมัติ ${row.exp_no}`);
      onDone();
    } catch (e) {
      showError(e);
    } finally {
    ...
    <Card className={row.duplicate_of ? "border-rose-300" : undefined}>
      <CardContent className="space-y-2 p-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {conf !== null && (
            <Badge variant={conf >= 0.8 ? "default" : "secondary"}>
        ...
        <div className="flex gap-3">
          ...
          <div className="min-w-0 flex-1 text-sm">
            ...
            <div className="text-xs text-muted-foreground">
              ผู้เบิก: <b>{row.requester_name}</b> ·{" "}
              {new Date(row.created_at).toLocaleString("th-TH")}
            </div>
            <div className="text-xs text-muted-foreground">
              ก่อน VAT ฿{Number(row.subtotal).toLocaleString()} + VAT ฿
              {Number(row.vat_amount).toLocaleString()}
            </div>
            {row.reject_reason && (
              <div className="text-xs text-rose-600">เหตุผล: {row.reject_reason}</div>
            )}
        ...
        {paidDialog && (
          <Dialog open onOpenChange={setPaidDialog}>
            <DialogContent className="max-w-sm">
              <DialogHeader>
                <DialogTitle>บันทึกการจ่าย: {row.exp_no}</DialogTitle>
              </DialogHeader>
              <Input
                placeholder="ชื่อผู้จ่าย / วิธีจ่าย"
                value={paidBy}
