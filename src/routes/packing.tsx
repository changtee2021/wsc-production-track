...
function PackingPage() {
  ...
  useEffect(() => {
    ...
    verify({ data: { password: "" } })
      ...
      .catch((err) =>
        toast.error(err instanceof Error ? err.message : "เข้าสู่ระบบแพ็คของไม่สำเร็จ"),
      );
  }, [verify]);
  if (!authed) {
    return (
...
const VID_EXT: Record<string, string> = {
  "video/mp4": "mp4",
  "video/webm": "webm",
  "video/quicktime": "mov",
  "video/x-m4v": "m4v",
  ...
  useEffect(() => {
    (async () => {
      ...
      if (token) {
        tasks.push(
          listEmps({ data: { token } })
            .then((r) => setEmps(r.rows as PackingEmployee[]))
            .catch(() => {}),
        );
      }
            ...
            <SelectContent>
              {emps.length === 0 && (
                <div className="px-3 py-2 text-sm text-muted-foreground">
                  ยังไม่มีพนักงาน — ให้แอดมินเพิ่มในหน้าจัดการ
                </div>
              )}
              {emps.map((e) => (
...
function PackingRow({
  ...
  return (
    <div
      className={`rounded-2xl border-2 bg-card p-3 transition-colors ${passed ? "border-success/40 bg-success/5" : failed ? "border-destructive/40 bg-destructive/5" : "border-border"}`}
    >
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <Button
          type="button"
          onClick={() => onPass(true)}
          className={`h-11 gap-1 text-sm font-semibold ${passed ? "bg-success text-success-foreground hover:bg-success/90" : "bg-success/15 text-success hover:bg-success/25"}`}
        >
        ...
        <Button
          ...
          className={`h-11 gap-1 text-sm font-semibold ${failed ? "bg-destructive text-destructive-foreground hover:bg-destructive/90" : "bg-destructive/15 text-destructive hover:bg-destructive/25"}`}
        >
