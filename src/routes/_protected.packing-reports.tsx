...
export const Route = createFileRoute("/_protected/packing-reports")({
  head: () => ({ meta: [{ title: "รายงานแพ็คของ — WSC ProductionTrack" }] }),
  component: PackingReportsPage,
});
...
function PackingReportsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const fetched = (res.rows ?? []) as unknown as PackingReportRow[];
      setRows(fetched);
      ...
      if (refs.size > 0) {
        try {
          const { urlMap } = await signUrls({
            data: { token, refs: Array.from(refs), defaultBucket: "packing-media" },
          });
          setSignedMap(urlMap);
        } catch {
  ...
  const handleExport = () => {
    ...
    downloadPackingReportsCsv(rows);
    ...
    <main className="mx-auto max-w-6xl px-4 py-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold tracking-tight">รายงานแพ็คของ</h1>
        </div>
        <Button
          variant="outline"
          className="gap-2"
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        รายงานข้อผิดพลาดจากพนักงานแพ็คของ พร้อมรูปและวิดีโอประกอบ
      </p>
                                      ...
                                      {it.media.map((m, i) => (
                                        ...
                                        >
                                          {m.type === "image" ? (
                                            ...
                                            <span className="absolute inset-0 flex flex-col items-center justify-center gap-1 bg-slate-800 text-white">
                                              <VideoIcon className="h-6 w-6" />
                                              <span className="text-[10px] font-medium">วิดีโอ</span>
                                            </span>
                                          )}
                                        </button>
                                      ))}
