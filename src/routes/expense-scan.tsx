...
import {
  issueExpenseSession,
  expenseUploadReceipt,
  expenseScanReceipt,
  expenseSubmit,
  expenseListEmployees,
  expenseListCategories,
  expenseCheckDuplicate,
...
function Form() {
  ...
  const [dupes, setDupes] = useState<
    Array<{ exp_no: string; requester_name: string; total_amount: number }>
  >([]);
  ...
  const runScan = async () => {
    if (!files.length) {
      toast.error("เพิ่มรูปใบเสร็จก่อน");
      return;
    }
    setScanning(true);
    try {
  ...
  const doSubmit = async () => {
    if (!empId) {
      toast.error("เลือกชื่อผู้เบิกก่อน");
      return;
    }
    if (!files.length) {
      toast.error("เพิ่มรูปใบเสร็จก่อน");
    ...
    <main className="min-h-screen bg-muted/30 pb-24">
      ...
      <div className="mx-auto max-w-md space-y-3 p-3">
        ...
        <Card>
          <CardContent className="space-y-2 p-3">
            ...
            {dupes.length > 0 && (
              <div className="flex items-start gap-2 rounded border border-rose-300 bg-rose-50 p-2 text-xs text-rose-900">
                ...
                <div>
                  ...
                  {dupes.map((d) => (
                    <div key={d.exp_no}>
                      · {d.exp_no} โดย {d.requester_name} ฿{Number(d.total_amount).toLocaleString()}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <Label className="text-xs">เลขที่บิล</Label>
                <Input value={receiptNo} onChange={(e) => setReceiptNo(e.target.value)} />
              </div>
              <div>
                <Label className="text-xs">วันที่บิล</Label>
                <Input
                  type="date"
                  value={receiptDate}
              ...
              <div>
                <Label className="text-xs">ก่อน VAT</Label>
                <Input
                  type="number"
                  step="0.01"
                  value={subtotal}
            ...
            <div className="flex items-center gap-2 text-xs">
              <input
                type="checkbox"
                id="wsc"
                checked={buyerMatchWsc}
                onChange={(e) => setBuyerMatchWsc(e.target.checked)}
              />
        ...
        <Button
          ...
          disabled={submitting || !empId || !files.length || !totalAmount}
          onClick={doSubmit}
        >
