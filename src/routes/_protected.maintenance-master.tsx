...
function MasterPage() {
  return (
    <main className="mx-auto max-w-4xl px-4 py-6">
      ...
      <p className="mb-4 text-sm text-muted-foreground">เพิ่ม/แก้ไขรายการทรัพย์สิน เครื่องจักร และสต๊อกอะไหล่ พร้อมแนบรูปประกอบ</p>
      <Tabs defaultValue="assets">
        ...
        <TabsContent value="parts" className="mt-3"><PartsPanel /></TabsContent>
      </Tabs>
    </main>
  );
}
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="mr-1 h-4 w-4" />เพิ่มทรัพย์สิน</Button>
      </div>
            ...
            <Card key={a.id}>
              <CardContent className="flex items-start gap-3 p-3">
                {a.image_url ? (
                  ...
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded border bg-muted text-muted-foreground"><Boxes className="h-6 w-6" /></div>
                )}
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">{a.category}{a.location && ` · ${a.location}`}{a.brand && ` · ${a.brand}`} {a.model}</div>
                  {(a.purchase_date || a.warranty_until) && (
                    <div className="text-xs text-muted-foreground">{a.purchase_date && `ซื้อ ${a.purchase_date}`} {a.warranty_until && `· ประกัน ${a.warranty_until}`}</div>
                  )}
                </div>
      ...
      {open && <AssetDialog initial={editing} onClose={() => { setOpen(false); refresh(); }} />}
    </div>
  );
}
      ...
      <DialogContent className="max-h-[90dvh] max-w-md overflow-y-auto">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {f.image_url ? (
                ...
                <div className="flex h-20 w-20 items-center justify-center rounded border bg-muted text-muted-foreground"><Boxes className="h-7 w-7" /></div>
              )}
              <div className="flex flex-col gap-1">
                ...
                {f.image_url && <Button type="button" size="sm" variant="ghost" onClick={() => setF({ ...f, image_url: null })}>ลบรูป</Button>}
              </div>
            </div>
          </div>
        ...
        <DialogFooter>
          ...
          <Button disabled={!f.name} onClick={async () => {
            try {
              await save({ data: { token: requireToken(), asset: { ...f, id: f.id || undefined } as Asset & { id?: string } } });
...
function PartsPanel() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex justify-end">
        <Button onClick={() => { setEditing(null); setOpen(true); }}><Plus className="mr-1 h-4 w-4" />เพิ่มอะไหล่</Button>
      </div>
      {rows.length === 0 ? <p className="text-center text-sm text-muted-foreground">— ยังไม่มี —</p> :
        <div className="space-y-2">
          {rows.map((p) => {
            ...
            return (
              <Card key={p.id}>
                <CardContent className="flex items-start gap-3 p-3">
                  {p.image_url ? (
                    <img src={p.image_url} alt={p.name} className="h-16 w-16 shrink-0 rounded border object-cover" />
                  ) : (
                    <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded border bg-muted text-muted-foreground"><Package2 className="h-6 w-6" /></div>
                  )}
                  <div className="min-w-0 flex-1">
                  ...
                  <div className="flex gap-1">
                    <Button size="sm" variant="outline" onClick={async () => {
                      ...
                      try { await restock({ data: { token: requireToken(), spare_part_id: p.id, delta: n, reason: "restock" } }); toast.success("เติมแล้ว"); refresh(); }
                      catch (e) { showError(e); }
                    }}>+เติม</Button>
                    <Button size="sm" variant="outline" onClick={() => { setEditing(p); setOpen(true); }}>แก้ไข</Button>
                    <Button size="icon" variant="ghost" onClick={async () => {
                      if (!confirm(`ลบ ${p.name}?`)) return;
                      try { await del({ data: { token: requireToken(), id: p.id } }); toast.success("ลบแล้ว"); refresh(); }
                      catch (e) { showError(e); }
                    }}><Trash2 className="h-4 w-4 text-rose-600" /></Button>
                  </div>
                </CardContent>
      ...
      {open && <PartDialog initial={editing} onClose={() => { setOpen(false); refresh(); }} />}
    </div>
  );
}
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-md">
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              {f.image_url ? (
                ...
                <div className="flex h-20 w-20 items-center justify-center rounded border bg-muted text-muted-foreground"><Package2 className="h-7 w-7" /></div>
              )}
              <div className="flex flex-col gap-1">
                <input ref={fileRef} type="file" accept="image/*" className="hidden"
                  onChange={(e) => { const file = e.target.files?.[0]; if (file) uploadImage(file); }} />
        ...
        <DialogFooter>
          ...
          <Button disabled={!f.name} onClick={async () => {
            try {
              await save({ data: { token: requireToken(), part: { ...f, id: f.id || undefined } as Part & { id?: string } } });
              toast.success("บันทึกแล้ว"); onClose();
            } catch (e) { showError(e); }
          }}>บันทึก</Button>
        </DialogFooter>
