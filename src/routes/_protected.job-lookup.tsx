...
import { adminFetchJobDetail } from "@/lib/features/admin.functions";
...
function JobLookupPage() {
  ...
  const downloadAllMedia = useCallback(async () => {
    ...
    for (const r of data.reports) {
      ...
      for (const it of r.qc_report_items ?? [])
        for (const m of it.media ?? []) push(m.url, "qc", m.type);
    }
    for (const r of data.packing_reports) {
      for (const m of r.media ?? []) push(m.url, "packing", m.type);
      for (const it of r.packing_report_items ?? [])
        for (const m of it.media ?? []) push(m.url, "packing", m.type);
    }
    if (items.length === 0) {
    ...
    try {
      ...
      await downloadMediaAsZip(data.job_id, downloadable, (d, t) =>
        setDownloadProgress({ done: d, total: t }),
      );
    ...
    <div className="mx-auto w-full max-w-6xl px-3 py-4 sm:px-6 sm:py-6">
      ...
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          ใส่เลข Job ID หรือสแกน QR เพื่อดูข้อมูลทุกอย่างของ Job ในหน้าเดียว
        </p>
      </div>
          ...
          <Card className="border-primary/30">
                    ...
                    <>
                      {qcBadge.pass > 0 && (
                        <Badge className="gap-1 bg-emerald-600 hover:bg-emerald-600">
                          <CheckCircle2 className="h-3 w-3" /> ผ่าน {qcBadge.pass}
                        </Badge>
                      )}
                      {qcBadge.fail > 0 && (
                        <Badge variant="destructive" className="gap-1">
                          <XCircle className="h-3 w-3" /> ไม่ผ่าน {qcBadge.fail}
                        </Badge>
                      )}
                      {qcBadge.unknown > 0 && (
                        <Badge variant="secondary" className="gap-1">
                          <HelpCircle className="h-3 w-3" /> ไม่ระบุ {qcBadge.unknown}
                        </Badge>
                      )}
                    </>
                  )}
            ...
            <CardContent>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                ...
                <Stat
                  icon={ListChecks}
                  label="ขั้นตอนที่เสร็จ"
                  value={detail.summary.steps_done}
                  sub={`${detail.summary.unique_steps} ขั้นตอน`}
                />
                  ...
                  {detail.logs.map((l) => {
                    ...
                    return (
                      <li key={l.id} className="relative">
                        <span
                          className={`absolute -left-[22px] top-1 inline-flex h-4 w-4 items-center justify-center rounded-full ring-2 ring-background ${a.cls}`}
                        >
                          <a.Icon className="h-3 w-3" />
                        </span>
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold ${a.cls}`}
                          >
                            {a.label}
                          </span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
                รายงาน QC ทั้งหมด ({detail.reports.length})
              </CardTitle>
            </CardHeader>
            <CardContent>
                        ...
                        <AccordionTrigger className="px-3 py-3 hover:no-underline">
                          <div className="flex flex-1 flex-wrap items-center gap-2 pr-2">
                            ...
                            <span className="ml-auto flex items-center gap-1.5">
                              ...
                              {r.status === "resolved" && (
                                <Badge variant="outline">แก้ไขแล้ว</Badge>
                              )}
                            </span>
                          </div>
                        </AccordionTrigger>
                                ...
                                {(r.qc_report_items ?? [])
                                  ...
                                  .map((it) => {
                                    ...
                                    return (
                                      <li
                                        key={it.id}
                                        className={`rounded-md border p-2 ${isMotor ? "border-amber-300 bg-amber-50/60 dark:bg-amber-950/20" : it.is_passed ? "border-emerald-200 bg-emerald-50/40 dark:bg-emerald-950/20" : "border-rose-200 bg-rose-50/40 dark:bg-rose-950/20"}`}
                                      >
                      ...
                      <AccordionItem key={r.id} value={r.id} className="rounded-lg border bg-card">
                        <AccordionTrigger className="px-3 py-3 hover:no-underline">
                          <div className="flex flex-1 flex-wrap items-center gap-2 pr-2">
                            ...
                            <span className="text-sm font-semibold">
                              ผู้แพ็ค: {r.packing_employees?.name ?? "-"}
                            </span>
                              ...
                              <ul className="space-y-2">
                                {items
                                  ...
                                  .map((it) => (
                                    <li
                                      key={it.id}
                                      className={`rounded-md border p-2 ${it.is_passed ? "border-emerald-200 bg-emerald-50/40 dark:bg-emerald-950/20" : "border-rose-200 bg-rose-50/40 dark:bg-rose-950/20"}`}
                                    >
                                      <div className="flex items-start gap-2">
                                        {it.is_passed ? (
                                          <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600" />
