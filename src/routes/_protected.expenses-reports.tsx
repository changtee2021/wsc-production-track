      ...
      {loading || !data ? (
        ...
        <>
          <h2 className="text-lg font-bold">
            รายงานค่าใช้จ่าย {data.period.month}/{data.period.year}
          </h2>

          <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
            <Kpi
              label="ค่าใช้จ่ายรวม (อนุมัติ+จ่ายแล้ว)"
              value={`฿${data.totals.total_expense.toLocaleString()}`}
            />
            <Kpi label="VAT รวม" value={`฿${data.totals.vat.toLocaleString()}`} />
            <Kpi
              label="ค่าเสื่อม/เดือน"
              value={`฿${data.totals.depreciation_monthly.toLocaleString()}`}
            />
            <Kpi
              label="รวมทั้งสิ้น"
              value={`฿${data.totals.grand_total.toLocaleString()}`}
              tone="primary"
            />
          </div>
              ...
              <Table>
                ...
                <TableBody>
                  {Object.entries(data.by_bill_type).map(([k, v]) => (
                    <TableRow key={k}>
                      <TableCell>{BILL_LBL[k] ?? k}</TableCell>
                      <TableCell className="text-right">{v.count}</TableCell>
                      <TableCell className="text-right">{v.vat.toLocaleString()}</TableCell>
                      <TableCell className="text-right">{v.total.toLocaleString()}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
                  ...
                  {data.depreciation_rows.map((r) => (
                    <TableRow key={r.id}>
                      ...
                      <TableCell className="text-right">
                        {r.purchase_price.toLocaleString()}
                      </TableCell>
  ...
  return (
    <Card>
      <CardContent className="p-3">
        ...
        <div className={`mt-1 text-xl font-bold ${tone === "primary" ? "text-primary" : ""}`}>
          {value}
        </div>
      </CardContent>
    </Card>
