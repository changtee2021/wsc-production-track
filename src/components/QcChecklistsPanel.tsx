...
export function QcChecklistsPanel() {
  const fetchChecklists = useServerFn(adminFetchChecklists);
  const upsert = useServerFn(adminUpsertChecklistItem);
  const del = useServerFn(adminDeleteChecklistItem);
  const reorder = useServerFn(adminReorderChecklist);
