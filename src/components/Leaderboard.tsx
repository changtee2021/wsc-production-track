...
export function Leaderboard({ className }: { className?: string }) {
  ...
  return (
    <section className={cn("rounded-3xl border border-border/60 bg-card p-5 shadow-[var(--shadow-card)]", className)}>
          ...
          {rows.map((r, i) => (
            <li key={r.employee_id} className={cn(
              ...
              <div className="w-6 text-center font-bold">
                {i < 3 ? <Medal className={cn("inline h-5 w-5",
                  i === 0 && "text-amber-500", i === 1 && "text-zinc-400", i === 2 && "text-orange-500")} /> : i + 1}
              </div>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="text-[11px] text-muted-foreground">
                  {r.finished} งาน{r.bonus > 0 && <> · <Sparkles className="inline h-3 w-3 text-amber-500" /> {r.bonus} โบนัส</>}
                </div>
              </div>
