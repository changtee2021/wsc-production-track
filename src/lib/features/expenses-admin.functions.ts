...
export const adminExpenseBulkApprove = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    for (const id of data.ids) {
      const { data: row } = await supabaseAdmin
        .from("expenses")
        .select("status, exp_no, requester_name, total_amount")
        .eq("id", id)
        .single();
...
export const adminExpenseMonthlyReport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    // by category
    const catIds = Array.from(
      new Set(approvedOrPaid.map((r) => r.category_id).filter(Boolean) as string[]),
    );
