// Server-only: build & send the daily LINE summary. Shared by:
...
function formatBangkokDate(): string {
  ...
  const yyyy = bkk.getUTCFullYear() + 543;
  return `${dd}/${mm}/${yyyy}`;
}

export function bangkokNowParts(): { hhmm: string; ymd: string } {
  const now = new Date();
  const bkk = new Date(now.getTime() + 7 * 60 * 60 * 1000);
