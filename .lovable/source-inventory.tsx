...
function StockInventoryAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const rows = await listFn({
        data: { adminToken: token, company, search: search.trim() || undefined },
      });
  ...
  const openEdit = (it: Item) => {
    setForm({
      ...
      unit: it.unit,
      total_qty: String(it.total_qty),
      min_safety_stock: String(it.min_safety_stock),
      max_stock_level: String(it.max_stock_level),
  ...
  const save = async () => {
    if (!form.item_code.trim() || !form.item_name.trim()) {
      toast.error("กรอกรหัสและชื่อสินค้า");
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="p-4 md:p-6 space-y-4 max-w-6xl mx-auto">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>{form.id ? "แก้ไขสินค้า" : "เพิ่มสินค้า"}</DialogTitle>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, min_safety_stock: e.target.value })}
              />
            </div>
            <div>
              <Label>Max</Label>
